...
| **Decoder** | iSTFTNet | iSTFTNet | HiFi-GAN |
| **multispeaker** | false | false | **true** |
...
Thứ ba, Stage 3 dùng schema khác (`config_ft.yml`) nên có vài điểm khác biệt: chỉ có trường `epochs` (không tách `epochs_1st/epochs_2nd`), `multispeaker: true`, decoder chuyển sang `hifigan`, và `load_only_params: true` để reset optimizer state khi chuyển sang domain mới.
