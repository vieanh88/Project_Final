...
def _detect_stage2_resume(stage2_log_dir: Path, logger: logging.Logger) -> Optional[str]:
    """
    Detect xem Stage 2 đã có step_2nd_*.pth hoặc epoch_2nd_*.pth chưa (RESUME case).
    Nếu có → trả về path tới file mới nhất, ngược lại None.
    """
    if not stage2_log_dir.exists():
        return None

    resume_files = list(stage2_log_dir.glob("step_2nd_*.pth")) + list(stage2_log_dir.glob("epoch_2nd_*.pth"))
    if not resume_files:
        return None

    # Mới nhất theo mtime; ưu tiên step checkpoint nếu nó là file mới nhất
    latest = sorted(resume_files, key=lambda f: f.stat().st_mtime, reverse=True)[0]
    logger.info(
        f"Phát hiện Stage 2 đã có {len(resume_files)} checkpoint resume "
        "(step_2nd_*.pth / epoch_2nd_*.pth)"
    )
...
def _detect_stage1_resume(stage1_log_dir: Path, logger: logging.Logger) -> Optional[str]:
    ...
    Ưu tiên file checkpoint theo thứ tự:
      1. step_1st_*.pth hoặc epoch_1st_*.pth mới nhất (theo mtime)
    ...
    # Ưu tiên 1: step_1st_*.pth hoặc epoch_1st_*.pth mới nhất
    # Cả hai đều có optimizer state; step checkpoint còn có batch_idx để skip batch đã chạy.
    resume_files = list(stage1_log_dir.glob("step_1st_*.pth")) + list(stage1_log_dir.glob("epoch_1st_*.pth"))
    if resume_files:
        latest = sorted(resume_files, key=lambda f: f.stat().st_mtime, reverse=True)[0]
        logger.info(
            f"Phát hiện Stage 1 đã có {len(resume_files)} checkpoint resume "
            "(step_1st_*.pth / epoch_1st_*.pth)"
        )
...
def auto_chain_checkpoint(
    ...
    Stage 1 (RESUME — đã có step_1st_*.pth hoặc epoch_1st_*.pth trong log_dir):
        - Set pretrained_model = latest step_1st_*.pth / epoch_1st_*.pth
    ...
    Stage 2 (RESUME — đã có step_2nd_*.pth hoặc epoch_2nd_*.pth trong log_dir):
        - PATH B: set pretrained_model = latest step_2nd_*.pth / epoch_2nd_*.pth +
    ...
    if stage == 1:
        ...
        # Detect resume: nếu Stage 1 đã có step_1st_*.pth hoặc epoch_1st_*.pth → RESUME
        if stage1_log_dir is not None:
            ...
            if resume_ckpt:
                ...
                # Chỉ cảnh báo khi resume từ epoch checkpoint.
                # Với step_1st_*.pth, số trong filename là global iters, không phải epoch.
    ...
    if stage == 2:
        ...
        # 2) Detect resume: nếu Stage 2 đã có step_2nd_*.pth hoặc epoch_2nd_*.pth → PATH B
